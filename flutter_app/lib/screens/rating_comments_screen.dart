...
class RatingCommentsScreen extends StatefulWidget {
  ...
  const RatingCommentsScreen({super.key, required this.rating});
...
class _RatingCommentsScreenState extends State<RatingCommentsScreen> {
  late Future<List<RatingComment>> _commentsFuture;
  ...
  Future<void> _reportRating() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: const Text('Reportar calificación'),
        content: TextField(
          controller: controller,
          maxLines: 4,
          decoration: const InputDecoration(
            labelText: 'Descripción',
            hintText: 'Explica por qué consideras inadecuada esta calificación',
          ),
        ),
        actions: [
          TextButton(
    ...
    try {
      await placeService.reportRating(
  ...
  Future<void> _reportComment(RatingComment comment) async {
    final placeService = Provider.of<PlaceService>(context, listen: false);
    final controller = TextEditingController();
  ...
  Future<void> _moderateComment(RatingComment comment, String action) async {
    final placeService = Provider.of<PlaceService>(context, listen: false);
    try {
      await placeService.moderateRatingComment(
        commentId: comment.id,
        action: action,
      );
      _loadComments();
  ...
  void _loadComments() {
    final placeService = Provider.of<PlaceService>(context, listen: false);
    setState(() {
      _commentsFuture = placeService.getCommentsForRating(widget.rating.id);
    });
  }
  ...
  Future<void> _sendComment() async {
    ...
    try {
      await placeService.addCommentToRating(
        ratingId: widget.rating.id,
        text: text,
      );
      _controller.clear();
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Comentarios de la calificación'),
      ),
      body: Column(
        children: [
          Padding(
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      tooltip: 'Reportar esta calificación',
                      onPressed: _reportRating,
                    ),
                  ],
                ),
          ...
          Expanded(
            child: FutureBuilder<List<RatingComment>>(
